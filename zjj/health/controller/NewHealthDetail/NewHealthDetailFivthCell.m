...
-(void)setInfoWithDict:(HealthDetailsItem *)item
{
    ...
    if (self.tag ==1) {//BMI 体重 体脂率
        self.image1.image = getImage(@"fat%1_");
        self.image2.image = getImage(@"fat1_");
        self.image3.image = getImage(@"fei1_");

        self.value1Label.text = [NSString stringWithFormat:@"体脂率(%.1f%%)",item.fatPercentage];
        self.status1Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_FATPERCENT item:item];


        self.value2Label.text = [NSString stringWithFormat:@"脂肪量(%.1fkg)",item.fatWeight];
        self.status2Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_FAT item:item];
        
        self.value3Label.text = [NSString stringWithFormat:@"内脏脂肪(%.1f)",item.visceralFatPercentage];
        ...
        self.image1.image = getImage(@"jirou1_");
        self.image2.image = getImage(@"danBZ1_");
        self.image3.image = getImage(@"bone1_");

        self.value1Label.text = [NSString stringWithFormat:@"肌肉(%.1f%%)",item.musclePercentage];
        self.status1Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_MUSCLE item:item];

        self.value2Label.text = [NSString stringWithFormat:@"蛋白质(%.1f%%)",item.proteinPercentage];
        self.status2Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_PROTEIN item:item];
        
        self.value3Label.text = [NSString stringWithFormat:@"骨量(%.1fkg)",item.boneWeight];
        self.status3Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_BONE item:item];


        
    }else{//肌肉  基础代谢  内脏脂肪
        self.image1.image = getImage(@"BMI1_");
        self.image2.image = getImage(@"boneMuscle1_");
        self.image3.image = getImage(@"water1_");

        self.value1Label.text = [NSString stringWithFormat:@"BMI(%.1f)",item.bmi];
        self.status1Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_BMI item:item];


        self.value2Label.text = [NSString stringWithFormat:@"骨骼肌(%.1f%%)",item.boneMusclePercentage];
        self.status2Label.text = [self getHealthDetailStatusTextWithStatus:IS_MODEL_BONEMUSCLE item:item];

        self.value3Label.text = [NSString stringWithFormat:@"水分(%.1f%%)",item.waterPercentage];

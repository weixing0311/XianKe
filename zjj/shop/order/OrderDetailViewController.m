...
-(void)getlistInfo_IS_CONSUMERS
{
    ...
    self.currentTasks = [[BaseSservice sharedManager]post1:@"app/order/queryOrderDetail.do" HiddenProgress:NO paramters:param success:^(NSDictionary *dic) {
        DLog(@"dic");
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        int status = [[_infoDict objectForKey:@"status"]intValue];
        NSString * clientDescription = [_infoDict safeObjectForKey:@"clientDescription"];
        if ((status==3&&clientDescription.length&&clientDescription.length>0)||status ==1) {
            return 80;
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section ==0) {
        ...
        NSString * clientDescription = [_infoDict safeObjectForKey:@"clientDescription"];
        if (status==3&&clientDescription.length&&clientDescription.length>0) {
            cell.hidden =NO;
